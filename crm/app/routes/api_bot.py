...
@api_bot_bp.route('/api/bot/qr', methods=['POST'])
def api_bot_qr_regenerate():
    """Trigger QR regeneration (async): clean old QR, restart bot, return OK."""
...
@api_bot_bp.route('/api/bot/qr-url')
def api_bot_qr_url():
    """Return QR image served directly from bot's local file."""
    from flask import send_file
    qr_local = os.path.join(BOT_DIR, 'qr-code.png')
    if os.path.exists(qr_local) and os.path.getsize(qr_local) > 100:
        age = _time_module.time() - os.path.getmtime(qr_local)
        return jsonify({'url': f'/api/bot/qr-img?_t={int(age)}', 'local': True, 'ok': True, 'age_sec': int(age)})
    return jsonify({'url': None, 'local': False, 'ok': True, 'message': 'No QR generado aún'})


@api_bot_bp.route('/api/bot/qr-img')
def api_bot_qr_img():
    """Serve the QR image directly from bot's filesystem."""
    from flask import send_file
    qr_local = os.path.join(BOT_DIR, 'qr-code.png')
    if os.path.exists(qr_local) and os.path.getsize(qr_local) > 100:
        resp = send_file(qr_local, mimetype='image/png')
        resp.headers['Cache-Control'] = 'no-cache, no-store, must-revalidate'
        return resp
    return jsonify({'error': 'QR no disponible'}), 404
