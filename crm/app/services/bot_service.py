...
def bot_action(action, payload_data=None):
    ...
    try:
        ...
        with urllib.request.urlopen(req, timeout=10) as resp:
            ...
            # Override 'connected' with real WhatsApp status from log
            try:
                ...
                if os.path.exists(log_path):
                    with open(log_path, 'rb') as f:
                        ...
                        if '✅ Bot HTK conectado y listo' in chunk:
                            result['whatsapp'] = 'connected'
                        elif '🔐 QR generado' in chunk or '🔐 Sesión autenticada' in chunk:
                            result['whatsapp'] = 'qr_pending' if 'QR generado' in chunk else 'authenticated'
                        else:
                            result['whatsapp'] = 'disconnected'
            except Exception as e:
                print(f'[bot_service] status log check error: {e}')
            return result
    except Exception as e:
        return {'ok': False, 'error': str(e)}
